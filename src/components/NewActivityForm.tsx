...
export default function NewActivityForm() {
  ...
  return (
    <div className="space-y-4">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {inputType === 'typed' ? (
          ...
          <div className="w-full p-6 border-2 border-dashed border-gray-300 rounded-md flex flex-col items-center">
            ...
            <span className="text-sm text-gray-500">Funcionalidade de OCR em desenvolvimento...</span>
          </div>
        )}
      ...
      {result && (
        <div className={`p-4 rounded-md border ${result.score >= 60 ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
          <div className="flex items-center gap-2 mb-2">
            {result.score >= 60 ? <CheckCircle className="text-green-600" /> : <AlertCircle className="text-red-600" />}
            <h3 className={`font-semibold ${result.score >= 60 ? 'text-green-800' : 'text-red-800'}`}>
              Nota: {result.score}
            </h3>
          </div>
