        ...
        <div className="p-5 border-b border-[#1e295b]/50 bg-[#030712]/50 flex items-center justify-between">
          <h3 className="font-bold text-white text-lg font-display">
            ...
            {activeTab === "resource_metrics" &&
              "Monitor de Gargalos de Recursos"}
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-700/50 pb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <h4 className="text-sm font-bold font-mono text-slate-200">
                        {sandboxMetrics.activity_name ||
                          "Diagnóstico de Submissões e Sandbox"}
                      </h4>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      <div className="flex flex-col">
                        <label className="text-[9px] font-mono uppercase tracking-wider text-slate-400 mb-0.5">
                        ...
                        >
                          <option value="all">
                            Todas as Atividades (Geral)
                          </option>
                          <option value="condicionais">
                            Atividade 1 (Simples)
                          </option>
                          <option value="lacos">Atividade 2 (Média)</option>
                          <option value="recursao">
                            Atividade 3 (Complexa)
                          </option>
                          <option value="grafos">
                            Atividade 4 (Muito Complexa)
                          </option>
                        </select>
                      </div>
                      ...
                      <div className="flex flex-col">
                        <label className="text-[9px] font-mono uppercase tracking-wider text-slate-400 mb-0.5">
                          Limite de Alerta de Timeout ({timeoutThreshold}%)
                        </label>
                        <div className="flex items-center gap-2">
                          <input
                            type="range"
                            ...
                            onChange={(e) =>
                              setTimeoutThreshold(parseFloat(e.target.value))
                            }
                            className="w-24 h-1 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-indigo-500"
                          />
                          <span className="text-xs font-mono text-indigo-400 font-bold min-w-[28px]">
                            {timeoutThreshold}%
                          </span>
                        </div>
                      </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="flex flex-col bg-sky-500/5 p-3 rounded-lg border border-sky-500/20">
                      ...
                      <span className="text-xl font-bold text-sky-300">
                        {sandboxMetrics.code_specific_issues}
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="p-3 bg-amber-500/10 border border-amber-500/30 rounded-lg flex items-start gap-3 mt-2">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-amber-400/80 mt-1">
                          Esta atividade ultrapassou a taxa limite de
                          tolerância. Sugerimos ajustar ou aumentar os limites
                          de tempo e RAM na configuração do Sandbox para
                          acomodar a complexidade do código submetido para
                          diminuir falsos positivos causados por Timeouts.
                        </span>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex flex-col mt-2">
                    <div className="flex items-center justify-between text-[10px] uppercase font-mono text-slate-400 mb-1">
                      ...
                      <span>
                        {Math.round(
                          ((sandboxMetrics.sandbox_timeouts +
                            sandboxMetrics.resource_limit_hits) /
                            (sandboxMetrics.sandbox_timeouts +
                              sandboxMetrics.resource_limit_hits +
                              sandboxMetrics.code_specific_issues)) *
                            100,
                    ...
                    <div className="w-full h-2.5 bg-[#0f172a] rounded-full overflow-hidden flex border border-slate-700/50">
                      <div
                        style={{
                          width: `${(sandboxMetrics.internal_errors / sandboxMetrics.total_executions) * 100}%`,
                        }}
                      ...
                      <div
                        style={{
                          width: `${(sandboxMetrics.sandbox_timeouts / sandboxMetrics.total_executions) * 100}%`,
                        }}
                      ...
                      <div
                        style={{
                          width: `${(sandboxMetrics.code_specific_issues / sandboxMetrics.total_executions) * 100}%`,
                        }}
                      ...
                      <div
                        style={{
                          width: `${((sandboxMetrics.total_executions - sandboxMetrics.internal_errors - sandboxMetrics.sandbox_timeouts - sandboxMetrics.code_specific_issues) / sandboxMetrics.total_executions) * 100}%`,
                        }}
                    ...
                    <p className="text-[10px] text-slate-500 mt-2 text-right">
                      Isso ajuda a identificar se as falhas na submissão ocorrem
                      devido a restrições de sandbox, como Timeouts e Limite de
                      RAM, em vez de erros lógicos dos aprendizes.
                    </p>
                  </div>
                </div>
              )}
                ...
                {alerts.map((a) => (
                  ...
                  >
                    <div className="flex-1">
                      <span className="text-sm font-bold font-mono uppercase tracking-wider text-slate-200">
                        {a.title}
                      </span>
            ...
            <div className="flex flex-col gap-6 animate-fade-in text-slate-200 animate-duration-300">
              ...
              <div className="bg-slate-900/60 p-5 rounded-2xl border border-slate-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    Visualização agregada de falhas de ambiente (
                    <strong className="text-rose-400 font-mono">
                      Internal Error
                    </strong>
                    ) e processos que excederam o tempo limite (
                    <strong className="text-amber-400 font-mono">
                      Sandbox Timeout
                    </strong>
                    ) para identificar gargalos críticos de recursos
                    computacionais.
                  </p>
                </div>
                ...
                <div className="flex flex-col gap-1 shrink-0">
                  ...
                  >
                    <option value="all">
                      Todas as Atividades (Métrica Global)
                    </option>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="bg-[#030712]/50 p-6 rounded-2xl border border-slate-700/40 flex flex-col justify-between min-h-[180px]">
                    <div>
                      ...
                      <p className="text-xs text-slate-300 mt-3 font-medium">
                        ...
                        de todas as {sandboxMetrics.total_executions} execuções.
                      </p>
                    </div>
                  ...
                  <div className="bg-[#030712]/50 p-6 rounded-2xl border border-slate-700/40 flex flex-col justify-between min-h-[180px]">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-3 font-normal">
                        Falhas severas de orquestração do sandbox ao iniciar
                        sub-processores ou pods Docker. Significa erro interno
                        ao provisionar ou isolar o ambiente.
                      </p>
                    </div>
                  ...
                  <div className="bg-[#030712]/50 p-6 rounded-2xl border border-slate-705 flex flex-col justify-between min-h-[180px]">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-3 font-normal">
                        Sinaliza que o script do teste consumiu mais tempo que a
                        janela máxima estrita (3.5s). Geralmente induzido por
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 font-normal">
                ...
                <div className="bg-[#030712]/50 p-6 rounded-2xl border border-slate-700/40 flex flex-col gap-4">
                  <div>
                    <h4 className="text-xs font-bold font-mono text-slate-400 uppercase tracking-wider">
                      Comparativo de Gargalos por Atividade
                    </h4>
                    <p className="text-xs text-slate-400 mt-1">
                      Análise comparativa das falhas de recursos acumuladas
                      (Internal Errors + Sandbox Timeouts).
                    </p>
                  </div>
                  ...
                  <div className="flex flex-col gap-3 mt-2">
                    {[
                      {
                        id: "all",
                        label: "Todas as Atividades (Métrica Global)",
                        internal: 12,
                        timeout: 45,
                        total: 1200,
                      },
                      {
                        id: "condicionais",
                        label: "Atividade 1: Est. Condicionais (Simples)",
                        internal: 1,
                        timeout: 2,
                        total: 400,
                      },
                      {
                        id: "lacos",
                        label: "Atividade 2: Laços de Repetição (Média)",
                        internal: 2,
                        timeout: 5,
                        total: 350,
                      },
                      {
                        id: "recursao",
                        label: "Atividade 3: Funções Recursivas (Complexa)",
                        internal: 4,
                        timeout: 15,
                        total: 250,
                      },
                      {
                        id: "grafos",
                        label:
                          "Atividade 4: Algoritmos de Grafos (Muito Complexa)",
                        internal: 5,
                        timeout: 23,
                        total: 200,
                      },
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between text-xs">
                            <span
                              className={`font-mono text-xs ${isSelected ? "text-indigo-300 font-bold" : "text-slate-300"}`}
                            >
                              {act.label}
                            </span>
                          ...
                          <div className="w-full bg-[#0f172a] border border-slate-800/80 rounded-full h-2.5 overflow-hidden flex">
                            <div
                              style={{
                                width: `${(act.internal / act.total) * 100}%`,
                              }}
                            ...
                            <div
                              style={{
                                width: `${(act.timeout / act.total) * 100}%`,
                              }}
                ...
                <div className="bg-[#030712]/50 p-6 rounded-2xl border border-slate-700/40 flex flex-col gap-4 justify-between">
                  <div className="flex flex-col gap-3">
                    <div>
                      <h4 className="text-xs font-bold font-mono text-slate-400 uppercase tracking-wider">
                        Ações de Otimização Recomendadas
                      </h4>
                      <p className="text-xs text-slate-400 mt-1">
                        Reações preventivas automatizadas para contornar
                        lentidões do servidor ou códigos ineficientes dos
                        estudantes.
                      </p>
                    </div>
                    ...
                    <div className="flex flex-col gap-2 mt-1">
                      <div className="bg-slate-950/40 border border-slate-850 p-3 rounded-xl">
                        <span className="text-[10px] font-mono text-indigo-400 font-bold uppercase block tracking-wider mb-1">
                          Ação 1: Ajuste de Janela Limite
                        </span>
                        <p className="text-xs text-slate-300">
                          Atividades como recursão ou grafos possuem
                          complexidade computacional elevada. Recomenda-se
                          elevar o tempo limite sandbox padrão para{" "}
                      ...
                      <div className="bg-slate-950/40 border border-[#1e295b]/30 p-3 rounded-xl">
                        <span className="text-[10px] font-mono text-rose-400 font-bold uppercase block tracking-wider mb-1">
                          Ação 2: Auditoria de Recursão & Loops Infinitos
                        </span>
                        <p className="text-xs text-slate-300 font-normal">
                          Disparar alerta pedagógico instruindo os alunos que
                          estão gerando timeouts recorrentes a revisitarem as
                          condições de escape de seus algoritmos recursivos.
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          {activeTab === "communication" && (
            <div className="flex flex-col gap-4">
              {notifications.map((n) => (
                <div
                  key={n.id}
                  className="p-4 rounded-xl border bg-[#030712]/50 border-slate-700/50 flex flex-col gap-2"
                >
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-bold text-slate-200">
                      Destinatário: {n.target}
                    </span>
                    <span className="text-xs bg-indigo-505/10 text-indigo-400 border border-indigo-500/30 px-2 py-0.5 rounded font-mono">
                      {n.status}
                    </span>
                  </div>
