...
export default function ReportsInterventionsView({ featureFlags = {} }: any) {
  const [subTab, setSubTab] = useState<
    "student" | "class" | "coordinator" | "risk"
  >("class");
  ...
  const exportPDF = () => {
    if (subTab === "class" && classData) {
      exportClassReportToPDF(classData);
    } else if (subTab === "student" && studentData) {
      exportStudentReportToPDF(selectedStudent, studentData, aiOpinion);
    } else {
      alert(
        "Iniciando exportação segura de documento em PDF com carimbo institucional...",
      );
    }
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-white font-display">
            Relatórios Cultivados e Intervenção (Fase 05)
          </h2>
          <p className="text-sm text-slate-400 mt-1">
            Análises consolidadas, pareceres com IA e exportação institucional.
          </p>
        </div>
        <div className="flex items-center gap-2">
          {featureFlags.ENABLE_PDF_EXPORT && (
            <button
              onClick={exportPDF}
              className="px-3 py-2 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 font-mono text-xs rounded-xl flex items-center gap-2 border border-emerald-500/30 transition-colors"
            >
              ...
              <div className="flex flex-col gap-6">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div className="p-4 rounded-xl border border-[#1e295b]/30 bg-[#0f172a] text-center">
                    <span className="block text-xs uppercase font-mono text-slate-400 mb-1">
                      Média Geral
                    </span>
                  ...
                  <div className="p-4 rounded-xl border border-[#1e295b]/30 bg-[#0f172a] text-center">
                    ...
                    <span className="text-3xl font-bold font-mono text-rose-400">
                      {classData.reprobation_risk_rate}%
                    </span>
                  </div>
                  <div className="p-4 rounded-xl border border-[#1e295b]/30 bg-[#0f172a] text-center">
                    <span className="block text-xs uppercase font-mono text-slate-400 mb-1">
                  ...
                  <div className="p-5 rounded-xl border border-[#1e295b]/30 bg-[#0f172a]">
                    ...
                    <ul className="flex flex-col gap-2">
                      {classData.strong_competencies.map((c: string) => (
                        <li
                          key={c}
                          className="text-sm text-slate-300 px-3 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-lg"
                        >
                          {c}
                        </li>
                      ))}
                  ...
                  <div className="p-5 rounded-xl border border-[#1e295b]/30 bg-[#0f172a]">
                    ...
                    <ul className="flex flex-col gap-2">
                      {classData.weak_competencies.map((c: string) => (
                        <li
                          key={c}
                          className="flex items-center justify-between text-sm text-slate-300 px-3 py-2 bg-amber-500/10 border border-amber-500/20 rounded-lg"
                        >
                          <span>{c}</span>
                          {featureFlags.ENABLE_INTERVENTION_PLAN && (
                            <button
                              onClick={() => loadInterventionPlan(c)}
                              className="text-[10px] bg-amber-500/20 text-amber-300 px-2 py-1 rounded hover:bg-amber-500/40 transition"
                            >
              ...
              <div className="flex flex-col gap-6">
                <div className="bg-[#0f172a] rounded-xl border border-[#1e295b]/30 p-5 flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <label className="text-xs uppercase font-mono text-slate-400">
                      Estudante:
                    </label>
                    <select
                  ...
                  {featureFlags.ENABLE_AI_PEDAGOGICAL_OPINION && (
                    <button
                      onClick={loadAIOpinion}
                      className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold uppercase tracking-wider transition"
                    >
                      {loadingAI ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-[#0f172a] border border-[#1e295b]/30 p-5 rounded-xl">
                    <h4 className="font-bold text-xs uppercase font-mono text-slate-400 mb-4">
                ...
                {riskData.map((r: any, i: number) => (
                  <div
                    ...
                    className={`p-5 rounded-xl border flex flex-col md:flex-row md:items-center justify-between gap-4 ${
                      r.riskLevel === "Crítico"
                        ...
                        : r.riskLevel === "Alto risco"
                          ? "bg-amber-500/10 border-amber-500/30"
                          : r.riskLevel === "Médio risco"
                  ...
                  >
                    ...
                    <div className="shrink-0 flex items-center gap-3">
                      <span
                        className={`px-3 py-1 rounded text-[10px] font-mono uppercase font-bold border ${
                          /*...color mapping*/
                          r.riskLevel === "Crítico"
              ...
              <div className="flex flex-col gap-6">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="p-4 bg-[#0f172a] border border-[#1e295b] rounded-xl text-center">
                    ...
                    <span className="text-3xl font-bold font-mono tracking-tighter text-white">
                      {coordinatorData.total_students}
                    </span>
                  </div>
                  <div className="p-4 bg-[#0f172a] border border-[#1e295b] rounded-xl text-center">
                    <span className="text-xs uppercase font-mono text-slate-400 block mb-1">
                    ...
                    <span className="text-3xl font-bold font-mono tracking-tighter text-sky-400">
                      {coordinatorData.average_institutional_score}
                    </span>
                  </div>
                  <div className="p-4 bg-[#0f172a] border border-[#1e295b] rounded-xl text-center">
                    <span className="text-xs uppercase font-mono text-slate-400 block mb-1">
                    ...
                    <span className="text-3xl font-bold font-mono tracking-tighter text-emerald-400">
                      {coordinatorData.engagement_rate}%
                    </span>
                  </div>
                </div>

                <div className="bg-[#0f172a] p-5 rounded-xl border border-[#1e295b]">
                  <h3 className="font-bold text-sm tracking-widest text-slate-400 uppercase font-mono mb-4">
                    Desempenho por Turma
                  </h3>
                  <div className="flex flex-col gap-3">
                    {coordinatorData.classes_performance.map(
                      (c: any, i: number) => (
                        <div
                          key={i}
                          className="flex items-center justify-between p-3 bg-[#030712] rounded-lg border border-[#1e295b]/50"
                        >
                          <span className="text-sm font-bold text-slate-200">
                            {c.name}
                          </span>
                          <span
                            className={`text-sm font-mono font-bold ${c.score >= 80 ? "text-emerald-400" : c.score >= 70 ? "text-amber-400" : "text-rose-400"}`}
                          >
