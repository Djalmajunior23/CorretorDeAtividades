      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 rounded-2xl border border-[#1e295b]/30 bg-[#0f172a] p-6 flex flex-col gap-4 h-fit">
          <div className="border-b border-[#1e295b]/20 pb-3">
            ...
            <p className="text-xs text-slate-400 mt-1">Configure o escopo de geração do enunciado avaliativo.</p>
          </div>

          <div className="flex flex-col gap-3 mt-2">
            <div>
              <label className="text-xs font-bold text-slate-300 block mb-1">Tema / Assunto</label>
              <input 
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-bold text-slate-300 block mb-1">Linguagem-Alvo</label>
                <select 
              ...
              <div>
                <label className="text-xs font-bold text-slate-300 block mb-1">Dificuldade</label>
                <select 
            ...
            <div>
              <label className="text-xs font-bold text-slate-300 block mb-1">Competência Pedagógica</label>
              <input 
            ...
            <div>
              <label className="text-xs font-bold text-slate-300 block mb-1">Contexto Realista</label>
              <textarea 
        ...
        <div className="lg:col-span-8 rounded-2xl border border-[#1e295b]/30 bg-[#0f172a] p-0 flex flex-col h-full overflow-hidden">
          {!generatedData ? (
            <div className="flex-1 flex items-center justify-center flex-col p-12 text-center text-slate-400">
              ...
              <h3 className="font-bold text-slate-300 font-mono">Aguardando Parâmetros</h3>
              <p className="text-sm mt-2 max-w-md">Preencha o formulário e clique em gerar para que a inteligência artificial construa uma atividade completa com enunciado, rubricas e master-tests.</p>
            </div>
          ) : (
            <div className="flex-1 flex flex-col h-full overflow-y-auto scrollbar-thin">
              <div className="p-6 border-b border-[#1e295b]/30 bg-[#030712]/30 flex items-center justify-between sticky top-0 z-10 backdrop-blur-md">
                <div>
                  <h2 className="text-lg font-bold text-white font-display leading-none">{generatedData.title || "Título Gerado"}</h2>
                  <div className="flex gap-2 mt-2">
                    {(generatedData.tags || []).map((t: string, i: number) => (
                      <span key={i} className="px-2 py-0.5 rounded-full border border-sky-500/20 bg-sky-500/10 text-[10px] text-sky-400 font-mono">
                        {t}
                      </span>
                    ))}
                  </div>
              ...
              <div className="p-6 flex flex-col gap-6 relative">
                {/* Enunciado */}
                <div>
                  <h4 className="text-xs font-mono uppercase tracking-widest text-slate-400 mb-2">Enunciado / Problema</h4>
                  <p className="text-sm text-slate-200 leading-relaxed bg-[#030712] p-4 rounded-xl border border-[#1e295b]/30 break-words whitespace-pre-wrap">{generatedData.problem_description}</p>
                </div>

                {/* IO */}
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-[#030712] p-4 rounded-xl border border-[#1e295b]/30">
                    <h4 className="text-[11px] font-mono uppercase text-emerald-400 font-bold mb-1 border-b border-emerald-500/20 pb-1">Formato de Entrada</h4>
                    <p className="text-[13px] text-slate-300 mt-2">{generatedData.inputs_desc}</p>
                  </div>
                  <div className="bg-[#030712] p-4 rounded-xl border border-[#1e295b]/30">
                    <h4 className="text-[11px] font-mono uppercase text-rose-400 font-bold mb-1 border-b border-rose-500/20 pb-1">Formato de Saída</h4>
                    <p className="text-[13px] text-slate-300 mt-2">{generatedData.outputs_desc}</p>
                  </div>
                </div>

                {/* Solution */}
                <div>
                  <h4 className="text-xs font-mono uppercase tracking-widest text-slate-400 mb-2">Gabarito Esperado ({formData.language.toUpperCase()})</h4>
                ...
                {/* Rubrica */}
                <div>
                  <h4 className="text-xs font-mono uppercase tracking-widest text-slate-400 mb-2">Rubrica de Avaliação Sugerida</h4>
                ...
                <div className="border-t border-[#1e295b]/30 pt-6">
                  <h4 className="text-xs font-mono uppercase tracking-widest text-slate-400 mb-3 flex items-center justify-between">
                    ...
                    <span className="bg-[#1e293b] px-2 py-0.5 rounded text-white">{generatedData.test_cases?.length || 0} Registrados</span>
                  </h4>
                  
                  <div className="flex flex-col gap-3">
                    {generatedData.test_cases?.map((tc: any, i: number) => (
                      <div key={i} className={`flex items-start gap-3 p-3 rounded-xl border ${tc.is_hidden ? 'border-amber-500/30 lg-amber-500/5' : 'border-[#1e295b]/40'} bg-[#0a0f24]`}>
                        <div className={`mt-1 shrink-0 px-2 py-0.5 rounded text-[9px] font-mono uppercase font-bold tracking-wider ${tc.is_hidden ? 'bg-amber-500/20 text-amber-400' : 'bg-emerald-500/20 text-emerald-400'}`}>
