...
import { Sparkles, FileText, BrainCircuit, Users, Target, CheckCircle2, Send, Loader2, Save, History, Trash2, HelpCircle, CornerDownLeft, Copy, Check } from "lucide-react";
...
export default function AIAssistantView({ featureFlags }: any) {
  ...
  const tools = [
    ...
    { id: "lesson_plan", title: "Plano de Aula", icon: FileText, flags: ["ENABLE_AI_LESSON_PLANNER"] },
    { id: "activity", title: "Gerar Atividade", icon: CodeIcon, flags: ["ENABLE_AI_ACTIVITY_BUILDER"] },
    { id: "rubric", title: "Gerar Rubrica", icon: CheckCircle2, flags: ["ENABLE_AI_RUBRIC_BUILDER"] },
    { id: "simulated_exam", title: "Simulado de Prova", icon: Target, flags: ["ENABLE_AI_SIMULATED_EXAMS"] },
    { id: "recovery_plan", title: "Recuperação Paralela", icon: Users, flags: ["ENABLE_AI_RECOVERY_PLAN"] },
  ];

  const quickActions = [
    { text: "Explique complexidade assintótica O(log N) de forma simples.", tool: "chat" },
    { text: "Crie um plano de 2 horas sobre algoritmos de Ordenação Rápida.", tool: "lesson_plan" },
    { text: "Escreva uma lista com 2 exercícios práticos sobre matrizes bidimensionais.", tool: "activity" },
    { text: "Sugerir critérios de avaliação para código limpo em Python.", tool: "rubric" }
  ];
  ...
  const handleGenerate = async (type: string, promptText?: string) => {
    ...
    try {
      let endpoint = type === "chat" ? "/api/codecheck/module06/student-recommendation" : `/api/codecheck/module06/${type.replace("_", "-") === "activity" ? "activity-builder" : type.replace("_", "-")}`;
      ...
      <div className="w-full lg:w-72 shrink-0 flex flex-col gap-4 overflow-y-auto pr-1">
        ...
        <div className="p-4 rounded-2xl bg-gradient-to-br from-indigo-500/10 via-indigo-500/5 to-slate-950 border border-indigo-500/20 shadow-lg shadow-indigo-500/5">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h2 className="font-bold text-white tracking-widest text-xs uppercase font-mono">Copiloto Docente IA</h2>
          </div>
        ...
        <div className="flex flex-col gap-1.5 bg-[#050819] p-2.5 rounded-2xl border border-slate-800">
          <span className="text-[9px] font-mono font-bold text-slate-500 uppercase tracking-widest px-2.5 mb-2 block">Modelos Disponíveis</span>
          {tools.map(tool => {
            const isAvailable = tool.flags.every(f => (featureFlags as any)[f] !== false);
            if (!isAvailable) return null;
            const Icon = tool.icon;
            return (
              <button 
                key={tool.id}
                onClick={() => { setActiveTool(tool.id); setResult(null); setPrompt(""); }}
        ...
        <div className="flex flex-col gap-2 bg-[#050819] p-3.5 rounded-2xl border border-slate-800">
          <div className="flex items-center gap-1.5 pl-0.5 mb-1">
            ...
            <span className="text-[9px] font-mono font-bold text-slate-500 uppercase tracking-widest block font-sans">Sugestões Docente</span>
          </div>
          <div className="flex flex-col gap-1.5 font-sans">
            {quickActions
              .filter(act => act.tool === activeTool || activeTool === "chat")
              .map((act, idx) => (
                <button
                  key={idx}
        ...
        <div className="flex flex-col gap-2 bg-[#050819] p-3.5 rounded-2xl border border-slate-800">
          <div className="flex items-center justify-between pb-1.5 border-b border-slate-900 mb-1">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[9px] font-mono font-bold text-slate-500 uppercase tracking-widest block">Histórico</span>
            </div>
            {history.length > 0 && (
              <button 
          ...
          <div className="flex flex-col gap-1.5 max-h-[160px] overflow-y-auto scrollbar-thin">
            {history.length === 0 ? (
              <span className="text-[10px] text-slate-600 font-mono italic text-center py-2 font-sans">Sem histórico recente</span>
            ) : (
              history.map(item => (
                <button
                  key={item.id}
        ...
        <div className="flex-1 overflow-y-auto p-6 scrollbar-thin flex flex-col gap-6 justify-between">
          ...
          <div className="flex-1 flex flex-col gap-6">
            ...
            {!loading && !result && (
              <div className="flex-1 flex flex-col items-center justify-center text-center max-w-sm mx-auto py-12 gap-4">
                ...
                <div>
                  <h4 className="text-sm font-bold text-slate-200">Como posso ajudar hoje?</h4>
                  <p className="text-xs text-slate-500 leading-relaxed mt-1 font-sans">
                    Digite sua solicitação ou clique em uma das sugestões rápidas ao lado para iniciar um plano analítico.
                  </p>
                </div>
              </div>
            )}
            ...
            {loading && (
              <div className="flex-1 flex flex-col items-center justify-center py-16 gap-3 text-indigo-400">
                ...
                <span className="font-mono text-xs tracking-wider uppercase">Computando Diretrizes Pedagógicas...</span>
              </div>
            )}
                ...
                {result.error ? (
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-1 pr-8">
                      <span className="text-[9px] font-mono font-bold text-indigo-400 uppercase tracking-widest pl-1">Resposta do Copiloto IA</span>
                      <div className="bg-slate-900/60 border border-slate-800/80 rounded-2xl p-5 shadow-inner">
                        ...
                        <div className="flex flex-col gap-5">
                          {Object.entries(result).map(([key, value], i) => (
                            <div key={i} className="flex flex-col gap-2 border-b border-slate-900 pb-4 last:border-0 last:pb-0 font-sans">
                              <span className="text-[10px] font-mono font-black text-emerald-400 uppercase tracking-wider">{key.replace(/_/g, ' ')}</span>
                              ...
                              {Array.isArray(value) ? (
                                ...
                                <div className="text-xs text-slate-300 whitespace-pre-wrap leading-relaxed">{(value as string)}</div>
                              )}
                            </div>
                          ))}
                        </div>
          ...
          <div className="mt-auto border-t border-slate-900 pt-5">
            <div className="flex flex-col gap-2 pl-1">
              <label className="text-[9px] font-mono font-bold text-slate-500 uppercase tracking-widest">Sua instrução ou prompt</label>
              <div className="relative flex items-center">
                ...
                >
                  {loading ? <Loader2 className="w-4 h-4 animate-spin text-slate-950" /> : <CornerDownLeft className="w-4 h-4 text-slate-950 stroke-[2.5]" />}
                </button>
              </div>
              <span className="text-[9px] text-slate-600 font-mono text-right pr-1">Pressione Enter para enviar, Shift+Enter para quebra de linha.</span>
            </div>
          </div>
...
function CodeIcon(props: any) {
  return (
    <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round font-sans">
