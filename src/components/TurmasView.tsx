...
export default function TurmasView() {
  ...
  const [profile, setProfile] = useState<
    "professor" | "coordinator" | "admin" | "super_admin"
  >("professor");
  ...
  const classesMap = {
    ...
    admin: [
      ...
      {
        id: "class_5",
        name: "Cybersecurity Advanced 3B",
        students: 30,
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <span className="text-xs font-mono font-bold tracking-widest text-[#10b981] uppercase">
            FASE 6 / 7: Controle de Acesso e Perfis
          </span>
          <h1 className="text-3xl font-extrabold text-white tracking-tight font-display mt-0.5">
            Gestão de Turmas e Perfis Docentes
          </h1>
          <p className="text-sm text-slate-400 mt-1">
            Simule permissões institucionais baseadas nos níveis hierárquicos do
            SENAI e filtre dados analíticos em tempo real.
          </p>
        </div>
          ...
          <div className="p-6 rounded-2xl bg-[#0f172a] border border-slate-800">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-bold text-white text-base">
                Unidades de Atendimento sob Responsabilidade
              </h3>
              <span className="text-xs font-mono font-bold text-slate-400 uppercase">
                {currentClasses.length} Turmas Ativas
              </span>
            </div>
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex flex-col gap-0.5">
                      ...
                      {cls.schoolUnit && (
                        <span className="text-[10px] text-[#a5f3fc] font-mono leading-none mt-1">
                          Unidade: {cls.schoolUnit}
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-6">
                    <div className="text-right flex flex-col items-end">
                      <span className="text-[9px] font-mono font-bold text-slate-500 uppercase leading-none">
                        Aproveitamento Médio
                      </span>
                      <span
                        className={`text-xs font-mono font-bold mt-1 ${cls.avgGrade && parseFloat(cls.avgGrade) < 7.0 ? "text-rose-400" : "text-emerald-400"}`}
                      >
                    ...
                    {cls.alertCount !== undefined && (
                      <div className="flex items-center gap-1">
                        <AlertTriangle
                          className={`w-4 h-4 ${cls.alertCount > 0 ? "text-amber-500 animate-pulse" : "text-slate-600"}`}
                        />
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <div className="p-6 rounded-2xl bg-[#0f172a] border border-slate-800">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="p-3.5 rounded-xl bg-slate-900/60 border border-slate-800">
                <span className="text-[9px] font-mono uppercase text-slate-500 font-bold block mb-1">
                  Índice de Correção em D+1
                </span>
                <span className="text-xl font-extrabold text-white">88%</span>
                <span className="text-[9px] font-mono text-cyan-400 ml-1.5 font-bold">
                  ● EXCELENTE
                </span>
              </div>
            </div>
