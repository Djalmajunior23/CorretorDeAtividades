...
export default function PedagogicalReportsPage() {
  ...
  const { report = {} as any, items = [] as any[], insights = [] as any[] } = reportData || {};
        ...
        {reportData && (
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-xl font-bold text-white">{report?.title || 'Sem título'}</h2>
                <p className="text-sm text-slate-500">{report?.class_name || 'Sem turma'}</p>
              </div>
              <p className="text-sm text-slate-500">Gerado em: {report?.created_at ? new Date(report.created_at).toLocaleDateString('pt-BR') : '-'}</p>
            </div>
