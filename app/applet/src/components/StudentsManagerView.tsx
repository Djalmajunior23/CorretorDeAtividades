                ...
                {students.map(s => (
                  <tr key={s.id} className="border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors group">
                    ...
                    <td className="py-3 px-4 text-right">
                      <button onClick={() => { setEditId(s.id); setFormData({ ...s }); setShowModal(true); }} className="text-emerald-400/70 hover:text-emerald-400 p-1 1 mr-1">
          ...
          <div className="bg-[#0f172a] rounded-2xl w-full max-w-lg border border-slate-800 shadow-2xl p-6 flex flex-col gap-4">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div className="flex justify-end gap-3 mt-4 pt-4 border-t border-slate-800">
                <button type="button" onClick={() => setShowModal(false)} className="px-4 py-2 text-sm text-slate-300 hover:text-white">Cancelar</button>
                <button type="submit" className="px-5 py-2 text-sm font-bold bg-emerald-500 text-slate-900 rounded-lg hover:bg-emerald-600">Salvar Registro</button>
              </div>
            </form>
          </div>
        </div>
