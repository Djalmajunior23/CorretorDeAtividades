...
const targetMethod = `  const handleDeleteSession = async (id: string) => {
    if (
      !window.confirm(
...
const newMethod = `  const handleDeleteSession = async (id: string) => {
    if (
      !window.confirm(
        "Deseja realmente excluir este registro de aula de forma irreversível?",
      )
...
if (content.includes('const res = await fetch(apiUrl(`/api/codecheck/diary/sessions/${id}`), {') && content.includes('showToast("Aula excluída com sucesso.");')) {
  ...
  console.log("Method not found or slightly different.");
}
